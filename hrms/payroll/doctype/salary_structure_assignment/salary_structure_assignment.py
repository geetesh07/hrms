...
from nts.utils import cint, flt, get_link_to_form, getdate

from hrms.payroll.doctype.payroll_period.payroll_period import get_payroll_period


class DuplicateAssignment(nts.ValidationError):
	pass
...
class SalaryStructureAssignment(Document):
	...
	def validate_dates(self):
		joining_date, relieving_date = nts.db.get_value(
	...
	def validate_company(self):
		salary_structure_company = nts.db.get_value(
		...
		if self.company != salary_structure_company:
			nts.throw(
				_("Salary Structure {0} does not belong to company {1}").format(
					nts.bold(self.salary_structure), nts.bold(self.company)
				)
			)
	...
	def validate_income_tax_slab(self):
		...
		if tax_component and not self.income_tax_slab:
			nts.throw(
				_(
					...
					get_link_to_form("Salary Structure", self.salary_structure), nts.bold(tax_component)
				),
		...
		income_tax_slab_currency = nts.db.get_value("Income Tax Slab", self.income_tax_slab, "currency")
			...
			if not payroll_payable_account:
				payroll_payable_account = nts.db.get_value(
					"Account",
					{
						...
						"account_currency": nts.db.get_value("Company", self.company, "default_currency"),
						"is_group": 0,
					},
				)
	...
	def get_payroll_cost_center(self):
		payroll_cost_center = nts.db.get_value("Employee", self.employee, "payroll_cost_center")
		if not payroll_cost_center and self.department:
			payroll_cost_center = nts.db.get_value("Department", self.department, "payroll_cost_center")
	...
	def validate_cost_centers(self):
		...
		for entry in self.payroll_cost_centers:
			company = nts.db.get_value("Cost Center", entry.cost_center, "company")
			if company != self.company:
				nts.throw(
					_("Row {0}: Cost Center {1} does not belong to Company {2}").format(
						entry.idx, nts.bold(entry.cost_center), nts.bold(self.company)
					),
		...
		if total_percentage != 100:
			nts.throw(_("Total percentage against cost centers should be 100"))
	...
	def warn_about_missing_opening_entries(self):
		if (
			...
			msg = _(
				...
				nts.bold(_("Taxable Earnings Till Date")),
				nts.bold(_("Tax Deducted Till Date")),
			)
			nts.msgprint(
				msg,
				indicator="orange",
...
def get_employee_currency(employee):
	employee_currency = nts.db.get_value("Salary Structure Assignment", {"employee": employee}, "currency")
...
def get_tax_component(salary_structure: str) -> str | None:
	salary_structure = nts.get_cached_doc("Salary Structure", salary_structure)
