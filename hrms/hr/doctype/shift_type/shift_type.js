	...
	refresh: function (frm) {
		...
		frm.add_custom_button(__("Mark Attendance"), () => {
			if (!frm.doc.enable_auto_attendance) {
				...
				nts.throw(__("Please Enable Auto Attendance and complete the setup first."));
			}

			if (!frm.doc.process_attendance_after) {
				frm.scroll_to_field("process_attendance_after");
				nts.throw(__("Please set {0}.", [__("Process Attendance After").bold()]));
			}

			if (!frm.doc.last_sync_of_checkin) {
				frm.scroll_to_field("last_sync_of_checkin");
				nts.throw(__("Please set {0}.", [__("Last Sync of Checkin").bold()]));
			}
			...
			frm.call({
				...
				freeze: true,
				callback: () => {
					nts.msgprint(__("Attendance has been marked as per employee check-ins"));
				},
			});
		});
	},
