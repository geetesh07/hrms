...
class DailyWorkSummaryGroup(Document):
	def validate(self):
		if self.users:
			if not nts.flags.in_test and not is_incoming_account_enabled():
				nts.throw(
...
def trigger_emails():
	...
	groups = nts.get_all("Daily Work Summary Group")
	for d in groups:
		group_doc = nts.get_doc("Daily Work Summary Group", d)
		if (
...
def is_current_hour(hour):
	return nts.utils.nowtime().split(":")[0] == hour.split(":")[0]


def send_summary():
	"""Send summary to everyone"""
	for d in nts.get_all("Daily Work Summary", dict(status="Open")):
		daily_work_summary = nts.get_doc("Daily Work Summary", d.name)
		daily_work_summary.send_summary()


def is_incoming_account_enabled():
	return nts.db.get_value("Email Account", dict(enable_incoming=1, default_incoming=1))
