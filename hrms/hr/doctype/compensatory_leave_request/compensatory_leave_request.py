...
from nts.utils import add_days, cint, date_diff, format_date, get_url_to_list, getdate
...
class CompensatoryLeaveRequest(Document):
	def validate(self):
		...
		if self.half_day:
			if not self.half_day_date:
				nts.throw(_("Half Day Date is mandatory"))
			if not getdate(self.work_from_date) <= getdate(self.half_day_date) <= getdate(self.work_end_date):
				nts.throw(_("Half Day Date should be in between Work From Date and Work End Date"))
		validate_overlap(self, self.work_from_date, self.work_end_date)
		self.validate_holidays()
		self.validate_attendance()
		if not self.leave_type:
			nts.throw(_("Leave Type is mandatory"))

	def validate_attendance(self):
		attendance_records = nts.get_all(
			"Attendance",
			filters={
		...
		if half_days and (not self.half_day or getdate(self.half_day_date) not in half_days):
			nts.throw(
				...
				).format(", ".join([nts.bold(format_date(half_day)) for half_day in half_days]))
			)

		if len(attendance_records) < date_diff(self.work_end_date, self.work_from_date) + 1:
			nts.throw(_("You are not present all day(s) between compensatory leave request days"))
		...
		if len(holidays) < date_diff(self.work_end_date, self.work_from_date) + 1:
			if date_diff(self.work_end_date, self.work_from_date):
				msg = _("The days between {0} to {1} are not valid holidays.").format(
					nts.bold(format_date(self.work_from_date)),
					nts.bold(format_date(self.work_end_date)),
				)
			else:
				msg = _("{0} is not a holiday.").format(nts.bold(format_date(self.work_from_date)))
	...
	def on_submit(self):
		...
		if leave_period:
			...
			comp_leave_valid_from = nts.bold(format_date(comp_leave_valid_from))
			...
			nts.throw(msg, title=_("No Leave Period Found"))
	...
	def on_cancel(self):
		if self.leave_allocation:
			...
			if self.half_day:
				date_difference -= 0.5
			leave_allocation = nts.get_doc("Leave Allocation", self.leave_allocation)
			if leave_allocation:
				leave_allocation.new_leaves_allocated -= date_difference
	...
	def get_existing_allocation(self, comp_leave_valid_from: datetime.date) -> dict | None:
		leave_allocation = nts.db.get_all(
			"Leave Allocation",
			filters={
				"employee": self.employee,
		...
		if leave_allocation:
			return nts.get_doc("Leave Allocation", leave_allocation[0].name)

	def create_leave_allocation(self, leave_period, date_difference):
		is_carry_forward = nts.db.get_value("Leave Type", self.leave_type, "is_carry_forward")
