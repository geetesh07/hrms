...
class VehicleLog(Document):
	...
	def on_submit(self):
		nts.db.set_value("Vehicle", self.license_plate, "last_odometer", self.odometer)

	def on_cancel(self):
		distance_travelled = self.odometer - self.last_odometer
		if distance_travelled > 0:
			updated_odometer_value = (
				int(nts.db.get_value("Vehicle", self.license_plate, "last_odometer")) - distance_travelled
			)
			nts.db.set_value("Vehicle", self.license_plate, "last_odometer", updated_odometer_value)


@nts.whitelist()
def make_expense_claim(docname):
	expense_claim = nts.db.exists("Expense Claim", {"vehicle_log": docname})
	if expense_claim:
		nts.throw(_("Expense Claim {0} already exists for the Vehicle Log").format(expense_claim))

	vehicle_log = nts.get_doc("Vehicle Log", docname)
	...
	if not claim_amount:
		nts.throw(_("No additional expenses has been added"))

	exp_claim = nts.new_doc("Expense Claim")
