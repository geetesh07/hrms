...
nts.ui.form.on("Leave Policy Assignment", {
	onload: function (frm) {
	...
	set_effective_date: function (frm) {
		if (frm.doc.assignment_based_on == "Leave Period" && frm.doc.leave_period) {
			nts.model.with_doc("Leave Period", frm.doc.leave_period, function () {
				let from_date = nts.model.get_value(
					"Leave Period",
					frm.doc.leave_period,
					"from_date",
				);
				let to_date = nts.model.get_value(
					"Leave Period",
					frm.doc.leave_period,
					"to_date",
				);
			...
			nts.model.with_doc("Employee", frm.doc.employee, function () {
				let from_date = nts.model.get_value(
					"Employee",
					frm.doc.employee,
					"date_of_joining",
				...
				frm.set_value(
					"effective_to",
					nts.datetime.add_months(frm.doc.effective_from, 12),
				);
			});
		}
		frm.refresh();
