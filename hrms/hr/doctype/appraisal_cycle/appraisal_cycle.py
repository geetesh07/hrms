...
class AppraisalCycle(Document):
	...
	def set_employees(self):
		...
		if employees:
			...
			self.set("appraisees", [])
			nts.msgprint(_("No employees found for the selected criteria"))
	...
	def get_appraisal_template_map(self):
		designations = nts.get_all("Designation", fields=["name", "appraisal_template"])
		appraisal_templates = nts._dict()
	...
	def show_missing_template_message(self, raise_exception=False):
		...
		).format(f"""<a href='{nts.utils.get_url_to_list("Designation")}'>Designations</a>""")
	...
	def complete_cycle(self):
		...
		draft_appraisals = nts.db.count("Appraisal", {"appraisal_cycle": self.name, "docstatus": 0})

		if draft_appraisals:
			link = nts.utils.get_url_to_list("Appraisal") + f"?status=Draft&appraisal_cycle={self.name}"
			link = f"""<a href="{link}">documents</a>"""

			msg = _("{0} Appraisal(s) are not submitted yet").format(nts.bold(draft_appraisals))
			msg += "<br><br>"
			msg += _("Please submit the {0} before marking the cycle as Completed").format(link)
			nts.throw(msg, title=_("Unsubmitted Appraisals"))
...
def validate_active_appraisal_cycle(appraisal_cycle: str) -> None:
	if nts.db.get_value("Appraisal Cycle", appraisal_cycle, "status") == "Completed":
		msg = _("Cannot create or change transactions against an Appraisal Cycle with status {0}.").format(
			nts.bold(_("Completed"))
		)
		msg += "<br><br>"
		msg += _("Set the status to {0} if required.").format(nts.bold(_("In Progress")))
...
def get_appraisal_cycle_summary(cycle_name: str) -> dict:
	...
	summary["appraisees"] = nts.db.count(
		"Appraisal", {"appraisal_cycle": cycle_name, "docstatus": ("!=", 2)}
	)
	summary["self_appraisal_pending"] = nts.db.count(
...
def get_employees_without_feedback(cycle_name: str) -> int:
	Feedback = nts.qb.DocType("Employee Performance Feedback")
	Appraisal = nts.qb.DocType("Appraisal")
	count = Count("*").as_("count")

	filtered_records = SubQuery(
		nts.qb.from_(Feedback)
