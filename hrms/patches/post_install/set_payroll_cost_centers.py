...
def execute():
	nts.reload_doc("payroll", "doctype", "employee_cost_center")
	nts.reload_doc("payroll", "doctype", "salary_structure_assignment")

	employees = nts.get_all("Employee", fields=["department", "payroll_cost_center", "name"])

	employee_cost_center = {}
	for d in employees:
		cost_center = d.payroll_cost_center
		if not cost_center and d.department:
			cost_center = nts.get_cached_value("Department", d.department, "payroll_cost_center")

		if cost_center:
			employee_cost_center.setdefault(d.name, cost_center)

	salary_structure_assignments = nts.get_all(
	...
	for d in salary_structure_assignments:
		...
		if cost_center:
			assignment = nts.get_doc("Salary Structure Assignment", d.name)
