...
def execute():
	if not (nts.db.table_exists("Payroll Period") and nts.db.table_exists("Taxable Salary Slab")):
		return

	if nts.db.a_row_exists("Income Tax Slab"):
		return
	...
	for doctype in (
		...
		nts.reload_doc("Payroll", "doctype", doctype)

	standard_tax_exemption_amount_exists = nts.db.has_column(
	...
	for company in nts.get_all("Company"):
		payroll_periods = nts.db.sql(
			f"""
			SELECT
				{select_fields}
			FROM
	...
	if not nts.db.table_exists("Employee Tax Exemption Proof Submission"):
		return

	if not nts.db.has_column("Employee Tax Exemption Proof Submission", "income_from_other_sources"):
		return
	...
	for proof in proofs:
		if proof.income_from_other_sources:
			employee_other_income = nts.new_doc("Employee Other Income")
	...
	if not nts.db.table_exists("Employee Tax Exemption Declaration"):
		return

	if not nts.db.has_column("Employee Tax Exemption Declaration", "income_from_other_sources"):
		return

	declerations = nts.get_all(
