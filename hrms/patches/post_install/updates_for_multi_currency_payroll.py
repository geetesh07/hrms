...
def execute():
	nts.reload_doc("payroll", "doctype", "Salary Component Account")
	if nts.db.has_column("Salary Component Account", "default_account"):
	...
	for item in doctype_list:
		nts.reload_doc(item["module"], "doctype", item["doctype"])
	...
	nts.db.sql("update `tabEmployee Advance` set exchange_rate=1 where exchange_rate is NULL")

	# get all companies and it's currency
	all_companies = nts.db.get_all(
	...
	for d in all_companies:
		...
		if not default_payroll_payable_account:
			default_payroll_payable_account = nts.db.get_value(
				"Account",
				{
