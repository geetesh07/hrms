...
class SalaryStructure(Document):
				...
				if not row.amount_based_on_formula and row.formula:
					nts.msgprint(
						_(
							...
							table.capitalize(),
							row.idx,
							nts.bold(_("Amount Based on Formula")),
							nts.bold(row.salary_component),
						),
						title=_("Warning"),
						indicator="orange",
					)
	...
	def set_missing_values(self):
		...
		for table in ["earnings", "deductions"]:
			for d in self.get(table):
				component_default_value = nts.db.get_value(
					"Salary Component",
	...
	def validate_amount(self):
		if flt(self.net_pay) < 0 and self.salary_slip_based_on_timesheet:
			nts.throw(_("Net pay cannot be negative"))
			...
			for row in self.get(component_type):
				if (
					...
					message = _("Row #{0}: The {1} Component has the options {2} and {3} enabled.").format(
						...
						nts.bold(row.salary_component),
						nts.bold(_("Amount based on formula")),
						nts.bold(_("Depends On Payment Days")),
					)
					message += "<br><br>" + _(
					...
					).format(nts.bold(_("Depends On Payment Days")), nts.bold(row.salary_component))
					nts.throw(message, title=_("Payment Days Dependency"))
		...
		for component in self.earnings:
			if component.salary_component == self.salary_component:
				nts.msgprint(
					...
					).format(self.idx, nts.bold(self.salary_component)),
					title=_("Warning"),
					indicator="orange",
				)
				break
		...
		if self.earnings:
			...
			for earning_component in self.earnings:
				if earning_component.is_flexible_benefit == 1:
					have_a_flexi = True
					max_of_component = nts.db.get_value(
			...
			if have_a_flexi and flt(self.max_benefits) == 0:
				nts.throw(_("Max benefits should be greater than zero to dispense benefits"))
	...
	def assign_salary_structure(
		...
		if employees:
			...
			nts.msgprint(_("No Employee Found"))
...
def assign_salary_structure_for_employees(
	...
	if assignments:
		nts.msgprint(_("Structures have been assigned successfully"))
...
def create_salary_structure_assignment(
	...
	assignment = nts.new_doc("Salary Structure Assignment")

	if not payroll_payable_account:
		payroll_payable_account = nts.db.get_value("Company", company, "default_payroll_payable_account")
		if not payroll_payable_account:
			nts.throw(_('Please set "Default Payroll Payable Account" in Company Defaults'))

	payroll_payable_account_currency = nts.db.get_value(
		"Account", payroll_payable_account, "account_currency"
	)
	company_curency = prodman.get_company_currency(company)
...
def get_existing_assignments(employees, salary_structure, from_date):
	# nosemgrep: nts-semgrep-rules.rules.nts-using-db-sql
	salary_structures_assignments = nts.db.sql_list(
		f"""
...
def make_salary_slip(
	...
	if cint(as_print):
		...
		return nts.get_print(doc.doctype, doc.name, doc=doc, print_format=print_format)
	else:
		return doc
