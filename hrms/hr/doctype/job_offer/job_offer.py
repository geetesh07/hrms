...
class JobOffer(Document):
	def onload(self):
		employee = nts.db.get_value("Employee", {"job_applicant": self.job_applicant}, "name") or ""
		self.set_onload("employee", employee)

	def validate(self):
		self.validate_vacancies()
		job_offer = nts.db.exists(
	...
	def validate_vacancies(self):
		...
		if staffing_plan and check_vacancies:
			...
			if not staffing_plan.get("vacancies") or cint(staffing_plan.vacancies) - len(job_offers) <= 0:
				error_variable = "for " + nts.bold(self.designation)
				if staffing_plan.get("parent"):
					error_variable = nts.bold(get_link_to_form("Staffing Plan", staffing_plan.parent))

				nts.throw(_("There are no vacancies under staffing plan {0}").format(error_variable))
...
def update_job_applicant(status, job_applicant):
	if status in ("Accepted", "Rejected"):
		nts.set_value("Job Applicant", job_applicant, "status", status)
...
def get_staffing_plan_detail(designation, company, offer_date):
	...
	return nts._dict(detail[0]) if (detail and detail[0].parent) else None


@nts.whitelist()
def make_employee(source_name, target_doc=None):
	def set_missing_values(source, target):
		target.personal_email, target.first_name = nts.db.get_value(
...
def get_offer_acceptance_rate(company=None, department=None):
	...
	total_offers = nts.db.count("Job Offer", filters=filters)

	filters["status"] = "Accepted"
	total_accepted = nts.db.count("Job Offer", filters=filters)
