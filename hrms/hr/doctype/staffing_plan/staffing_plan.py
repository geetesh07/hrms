...
from nts.utils import cint, flt, getdate, nowdate
from nts.utils.nestedset import get_descendants_of


class SubsidiaryCompanyError(nts.ValidationError):
	pass


class ParentCompanyError(nts.ValidationError):
	pass
...
class StaffingPlan(Document):
	...
	def validate_with_parent_plan(self, staffing_plan_detail):
		if not nts.get_cached_value("Company", self.company, "parent_company"):
		...
		parent_company = nts.db.get_value("Staffing Plan", parent_plan_details[0].name, "company")
		...
		if cint(staffing_plan_detail.vacancies) > cint(parent_plan_details[0].vacancies) or flt(
			...
			nts.throw(
				_(
					...
					nts.bold(staffing_plan_detail.designation),
		...
		lft, rgt = nts.get_cached_value("Company", parent_company, ["lft", "rgt"])
		all_sibling_details = nts.db.sql(
...
def get_designation_counts(designation, company, job_opening=None):
	...
	job_openings = nts.db.count("Job Opening", filters)
...
def get_active_staffing_plan_details(company, designation, from_date=None, to_date=None):
	...
	if not company or not designation:
		nts.throw(_("Please select Company and Designation"))
	...
	if not staffing_plan:
		parent_company = nts.get_cached_value("Company", company, "parent_company")
		if parent_company:
