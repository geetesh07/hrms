...
from nts.utils import add_days, date_diff, format_date, get_link_to_form, getdate

from prodman.setup.doctype.employee.employee import is_holiday

from hrms.hr.utils import validate_active_employee, validate_dates


class OverlappingAttendanceRequestError(nts.ValidationError):
	pass
...
class AttendanceRequest(Document):
	...
	def validate_half_day(self):
		if self.half_day:
			if not getdate(self.from_date) <= getdate(self.half_day_date) <= getdate(self.to_date):
				nts.throw(_("Half day date should be in between from date and to date"))
	...
	def validate_request_overlap(self):
		...
		Request = nts.qb.DocType("Attendance Request")
		overlapping_request = (
			nts.qb.from_(Request)
			.select(Request.name)
			.where(
	...
	def throw_overlap_error(self, overlapping_request: str):
		...
		nts.throw(msg, title=_("Overlapping Attendance Request"), exc=OverlappingAttendanceRequestError)

	def on_submit(self):
		self.create_attendance_records()

	def on_cancel(self):
		attendance_list = nts.get_all(
			"Attendance", {"employee": self.employee, "attendance_request": self.name, "docstatus": 1}
		)
		if attendance_list:
			for attendance in attendance_list:
				attendance_obj = nts.get_doc("Attendance", attendance["name"])
		...
		if doc:
			...
			if old_status != status:
				...
				text = _("changed the status from {0} to {1} via Attendance Request").format(
					nts.bold(old_status), nts.bold(status)
				)
		...
		if not self.include_holidays and is_holiday(self.employee, attendance_date):
			nts.msgprint(
				_("Attendance not submitted for {0} as it is a Holiday.").format(
					nts.bold(format_date(attendance_date))
				)
			)
			return False
		...
		if self.has_leave_record(attendance_date):
			nts.msgprint(
				_("Attendance not submitted for {0} as {1} is on leave.").format(
					nts.bold(format_date(attendance_date)), nts.bold(self.employee)
				)
			)
			return False
	...
	def get_attendance_doc(self, attendance_date: str) -> str | None:
		...
		return nts.get_doc("Attendance", attendance) if attendance else None
