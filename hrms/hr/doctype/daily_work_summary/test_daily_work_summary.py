...
# test_records = nts.get_test_records('Daily Work Summary')


class TestDailyWorkSummary(ntsTestCase):
	...
	def test_email_trigger_failed(self):
		hour = "00:00"
		if nts.utils.nowtime().split(":")[0] == "00":
			hour = "01:00"
	...
	def test_incoming(self):
		...
		daily_work_summary = nts.get_doc(
			"Daily Work Summary", nts.get_all("Daily Work Summary")[0].name
		)
	...
	def setup_and_prepare_test(self, hour=None):
		nts.db.sql("delete from `tabDaily Work Summary`")
		nts.db.sql("delete from `tabEmail Queue`")
		nts.db.sql("delete from `tabEmail Queue Recipient`")
		nts.db.sql("delete from `tabCommunication`")
		nts.db.sql("delete from `tabDaily Work Summary Group`")

		self.users = nts.get_all("User", fields=["email"], filters=dict(email=("!=", "test@example.com")))
	...
	def setup_groups(self, hour=None):
		...
		if not hour:
			hour = nts.utils.nowtime().split(":")[0]
			hour = hour + ":00"
